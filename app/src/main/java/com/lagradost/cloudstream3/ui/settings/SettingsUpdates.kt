...
class SettingsUpdates : BasePreferenceFragmentCompat() {
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        // BACKUP
        getPref(R.string.backup_key)?.setOnPreferenceClickListener { BackupUtils.backup(activity); true }
        ...
        getPref(R.string.automatic_backup_key)?.setOnPreferenceClickListener {
            ...
            activity?.showDialog(prefNames.toList(), prefValues.indexOf(current),
                getString(R.string.backup_frequency), true, {}) { index ->
                settingsManager.edit { putInt(getString(R.string.automatic_backup_key), prefValues[index]) }
                BackupWorkManager.enqueuePeriodicWork(context ?: CloudStreamApp.context, prefValues[index].toLong())
            }
            true
        }

        getPref(R.string.redo_setup_key)?.setOnPreferenceClickListener { findNavController().navigate(R.id.navigation_setup_language); true }
        getPref(R.string.restore_key)?.setOnPreferenceClickListener { activity?.restorePrompt(); true }

        getPref(R.string.backup_path_key)?.hideOn(EMULATOR)?.setOnPreferenceClickListener {
            val dirs = getBackupDirsForDisplay()
            val currentDir = settingsManager.getString(getString(R.string.backup_dir_key), null)
        ...
        getPref(R.string.show_logcat_key)?.setOnPreferenceClickListener { pref ->
            ...
            try {
                ...
                BufferedReader(InputStreamReader(process.inputStream)).lineSequence().forEach { logList.add(it) }
            } catch (e: Exception) { logError(e) }

            binding.logcatRecyclerView.layoutManager = LinearLayoutManager(pref.context)
            binding.logcatRecyclerView.adapter = LogcatAdapter().apply { submitList(logList) }
            ...
            binding.saveBtt.setOnClickListener {
                ...
                try { VideoDownloadManager.setupStream(it.context, "logcat_$date", null, "txt", false).openNew().writer().use { w -> w.write(logList.joinToString("\n")) }; dialog.dismissSafe(activity) }
                catch (t: Throwable) { logError(t); showToast(t.message) }
            }
        ...
        getPref(R.string.install_prerelease_key)?.let { pref ->
            ...
            pref.setOnPreferenceClickListener { activity?.installPreReleaseIfNeeded(); true }
        }
        ...
        getPref(R.string.manual_update_plugins_key)?.setOnPreferenceClickListener {
            ioSafe { PluginManager.___DO_NOT_CALL_FROM_A_PLUGIN_manuallyReloadAndUpdatePlugins(activity ?: return@ioSafe) }
            true
        }
    }
    ...
    private fun getBackupDirsForDisplay(): List<String> {
        return safe {
            context?.let { ctx ->
                ...
                (runCatching { first + BackupUtils.getCurrentBackupDir(ctx).let { it.first?.filePath() ?: it.second } }.getOrNull() ?: first)
